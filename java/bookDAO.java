...
public class bookDAO {
    ...
    public static int  memberornot(String username) {
        int status = 0;
        int  a = 0;
        try {
            conn = ConnectionProvider.getCon();
            PreparedStatement pst = conn.prepareStatement("Select username from members where username=? and mem='yes'");
            pst.setString(1, username);
            ResultSet rs = pst.executeQuery();
             if(rs.next())
             {
    ...
    public static ArrayList<Object>takenbooks(String username)    {
        ArrayList<Object> books = new ArrayList<Object>();
        try {

            conn = ConnectionProvider.getCon();
            PreparedStatement pst = conn.prepareStatement("select*from issuedbooks where username=?");
            pst.setString(1, username);
            ResultSet rs = pst.executeQuery();

            while (rs.next()) {
                books.add(rs.getString("bookname"));
                books.add(rs.getString("untildate"));
            }
            conn.close();
        } catch (Exception ex) {
            System.out.println(ex);
